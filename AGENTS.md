...
## Dependencies
- Install local API client from `lib/openapi-java-client-0.0.10.jar` using the sidecar POM:
  `make install-client`
- The Makefile `install-client` target installs `0.0.10`. Always use the version in `pom.xml` and `install-client` as source of truth.
